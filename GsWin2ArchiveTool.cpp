...
std::string python_path = "C:\\Users\\julixian\\AppData\\Local\\Programs\\Python\\Python311\\python.exe";
...
private:
    /*std::vector<uint8_t> compress(const std::vector<uint8_t>& input) {
        std::ofstream outTmp("tmp.bin", std::ios::binary);
        outTmp.write((char*)input.data(), input.size());
        outTmp.close();
        std::string cmd = python_path + " lzss_pack.py tmp.bin tmp_compressed.bin";
        system(cmd.c_str());
        std::ifstream inTmp("tmp_compressed.bin", std::ios::binary);
        auto file_size = fs::file_size("tmp_compressed.bin");
        std::vector<uint8_t> output(file_size);
        inTmp.read((char*)output.data(), file_size);
        inTmp.close();
        fs::remove("tmp.bin");
        fs::remove("tmp_compressed.bin");
        return output;
    }*/
